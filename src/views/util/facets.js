...
function(Backbone, _, _s, FacetCollectionView, FacetsEditorView, FunctionsUtil, FiltersUtil, RequestsUtil, FormatUtil){
  ...
  var connectFacetsEditor = function(ctx, opts){
    ...
    // Connect the facetCollectionView .
    var facetCollectionView = facetsEditor.subViews.facets;
    if (facetCollectionView){

      // Connect existing facets.
      _.each(facetCollectionView.registry, function(facetView, id){
        connectFacet(facetView, ctx);
      });
  ...
  var updateFacetModelPrimaryFilters = function(facetModel, opts){
    var oldFilters = FiltersUtil.getModelFilters(facetModel, 'primary', opts);

    // Clone old filters if present.
    var newFilters = {};
    if (oldFilters){
    ...
    _.each(newFilters, function(filterSet, groupId){
      var keep_filters = [];
      ...
      newFilters[groupId] = keep_filters;
    });

    // Set filters if changed.
    if (! _.isEqual(oldFilters, newFilters)){
      facetModel.set('primary_filters', newFilters);
    }
  ...
  var connectFacet = function(facet, opts){
    ...
    if (facet.model.getData){
      facet.model.on('change:primary_filters change:base_filters change:quantity_field', _.throttle(function(){
        ...
        if (facet.model.get('type') == 'numeric' && changes && changes.changes 
